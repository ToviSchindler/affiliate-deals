...
const SHEET_NAME = 'AliExpress';

// הגדרת מבנה הנתונים לפי העמודות שיצרת בגיליון
interface Deal {
  id: string;
  title: string;
  link: string;
    ...
    <div className="pt-8 pb-16">
        ...
        {deals.map((deal: Deal, index: number) => (
          <div key={index} className="bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col">
            ...
            <div className="p-5 flex flex-col flex-grow gap-2">
              <span className="text-xs font-bold bg-orange-100 text-orange-800 px-3 py-1 rounded-full self-start">
                {deal.category || 'כללי'}
              </span>
              ...
              <div className="mt-auto pt-5 flex items-center justify-between">
                ...
                <a 
                  ...
                  className="bg-orange-500 hover:bg-orange-600 text-white font-bold py-2.5 px-6 rounded-xl transition-colors shadow-md"
                >
                  לקנייה
                </a>
              </div>
      ...
      {(!deals || deals.length === 0) && (
        <div className="text-center text-gray-500 mt-16 text-lg">
          עדיין אין דילים. הוסיפו שורה ללשונית AliExpress בגיליון!
        </div>
      )}
    </div>
  );
